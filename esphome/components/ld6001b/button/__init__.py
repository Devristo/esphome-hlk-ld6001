...
from .. import CONF_LD6001B_ID, LD6001BComponent, ld6001b_ns

DEPENDENCIES = ["ld6001b"]

StartButton = ld6001b_ns.class_("StartButton", button.Button)
StopButton = ld6001b_ns.class_("StopButton", button.Button)
HardResetButton = ld6001b_ns.class_("HardResetButton", button.Button)
SoftResetButton = ld6001b_ns.class_("SoftResetButton", button.Button)
FactoryResetButton = ld6001b_ns.class_("FactoryResetButton", button.Button)
...
async def to_code(config):
    ...
    if start_config := config.get(CONF_START):
        ...
        await cg.register_parented(b, config[CONF_LD6001B_ID])
        cg.add(ld6001b_component.set_start_button(b))
    if stop_config := config.get(CONF_STOP):
        b = await button.new_button(stop_config)
        await cg.register_parented(b, config[CONF_LD6001B_ID])
        cg.add(ld6001b_component.set_stop_button(b))
    if hard_reset_config := config.get(CONF_HARD_RESET):
        b = await button.new_button(hard_reset_config)
        await cg.register_parented(b, config[CONF_LD6001B_ID])
        cg.add(ld6001b_component.set_hard_reset_button(b))
    if soft_reset_config := config.get(CONF_SOFT_RESET):
        b = await button.new_button(soft_reset_config)
        await cg.register_parented(b, config[CONF_LD6001B_ID])
        cg.add(ld6001b_component.set_soft_reset_button(b))
    if factory_reset_config := config.get(CONF_FACTORY_RESET):
        b = await button.new_button(factory_reset_config)
        await cg.register_parented(b, config[CONF_LD6001B_ID])
        cg.add(ld6001b_component.set_factory_reset_button(b))
